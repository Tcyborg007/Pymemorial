...
class TestGetNaturalWriterConfig:
    ...
    def test_get_natural_writer_config_default_values(self):
        """
        Testa valores padrão retornados (integrados com core.config)
        
        NOTE: precision pode variar baseado em core/config.py:
        - NaturalWriterConfig default: 2
        - PyMemorialConfig global: 3 (NBR padrão)
        """
        config = get_natural_writer_config()
        
        assert config.style == NaturalLanguageStyle.TECHNICAL
        
        # Aceita precision de core.config (3) ou default (2)
        assert config.precision in [2, 3], (
            f"precision deve ser 2 (default) ou 3 (core.config), "
            f"recebido: {config.precision}"
        )
